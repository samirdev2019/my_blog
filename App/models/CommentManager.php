...
class CommentManager extends Database
{
    ...
    public function getComments(int $id):array
    {
        ...
        $req=$this->pdo->prepare('SELECT comment_id,content,validated,commented_by,post_id,date_format(commented,\' %d\%m\%Y à %Hh%imin%ss\') as coment_date
        FROM comments 
    ...
    {
        $comment=$this->pdo->prepare('INSERT INTO comments(commented,content,post_id,commented_by) VALUES (now(),:content,:post_id,:commented_by)');
    ...
    {   
        $req=$this->pdo->prepare('UPDATE comments SET validated =1,user_id =:userId WHERE comment_id = :comment_id');
        return $req->execute([
            'userId' => $userId,
    ...
    {
        $req=$this->pdo->prepare('SELECT comment_id,commented,commented_by,content,validated,post_id
        FROM comments  
