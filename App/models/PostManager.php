...
class PostManager extends Database
{
    ...
    {
        $posts = $this->db->query('SELECT post_id,title,author,chapo,content,date_format(updated,\' %d\%m\%Y à %Hh%imin%ss\') as update_date,user_id FROM posts ORDER BY updated DESC');
        $datas = $posts->fetchAll(\PDO::FETCH_OBJ);
        return $datas  ;
    }
    public function getPost(int $id):object
    {
        $post=$this->db->prepare('SELECT post_id,title,author,chapo,content,date_format(updated,\' %d\%m\%Y à %Hh%imin%ss\') as update_date,user_id FROM posts WHERE post_id = ?');
